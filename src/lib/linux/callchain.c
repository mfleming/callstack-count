 ...
 * Copyright (C) 2024, Matt Fleming <mfleming@cloudflare.com>
 *
 ...
 * 
 * Radix trees are used to model callchains. In this sense, we have
 * a simple string matching problem where each "character" in the string
 * is a 64-bit address and a string is the addresses that make up a callchain.
 * 
 * The callchain code requires the following properties from radix trees:
 * 
 * 1. Fast insertion and lookup
 * 2. A count maintained for each path to show how many samples had that callchain
 * 3. Point queries, e.g. "is this exact callchain in the tree?"
 *
 * Crucially, we don't need to:
 * 
 * 1. Maintain a balanced tree
 * 2. Support delete or range operations
 * 
 * We build *many* radix trees: one for each struct hist_entry (aka sample).
 * For large perf.data files this can mean we process millions callchains.
 * 
 * Bottlenecks
 * ===========
 * Because we don't need to delete or progressively update radix trees, the
 * bottleneck for this code is construction, specifically key comparison. 
 * 
 * SORTING
 * =======
 * We also need to build a second sorted rbtree from this radix tree. See
 * sort_chain_* below. Again, traversal performance for the radix tree is
 * critical.
 */
...
fill_node(struct callchain_node *node, struct callchain_cursor *cursor)
{
	...
	if (!node->val_nr) {
		...
		fprintf(stderr, "Empty node at fill");
	}
...
static void mjf2(void) {fprintf(stderr, "Somethings wrong");}
/*
		...
		u64 idx_parents, u64 idx_local, u64 period)
{
	...
	if (new->val_nr == 0) {
		fprintf(stderr, "Empty node value\n");
	}
	parent->val_nr = idx_local;
	if (parent->val_nr == 0) {
		fprintf(stderr, "Parenty empty value\n");
	}
...
// static void mjf(struct callchain_node *node) { fprintf(stderr, "Got zero %u", node->count);}
